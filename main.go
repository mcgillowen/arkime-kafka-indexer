...
// main package for the arkime-kafka-indexer
// contains the main func as well as some helper functions for setting up the indexer.
package main
...
func setupKafka(
	...
	useTLS := env.KafkaSSLCALocation != "" &&
		env.KafkaSSLCertLocation != "" &&
		env.KafkaSSLKeyLocation != "" &&
		env.KafkaSSLKeyPassword != ""
